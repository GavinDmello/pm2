...
var InteractorDaemon = module.exports = function () {
  ...
  this.transport.on('error', function (err) {
    return console.error('[TRANSPORT] Error : ' + err.message || err);
  });
...
InteractorDaemon.prototype.exit = function (err) {
  ...
  if (this._workerConnectivity) clearInterval(this._workerConnectivity);
  ...
  dns.lookup('google.com', function (err) {
    if (err && (err.code === 'ENOTFOUND' || err.code === 'EAI_AGAIN')) {
      ...
      if (!self._online) {
        ...
        // reconnect only if the connection is actually disconnected
...
InteractorDaemon.prototype._verifyEndpoint = function (cb) {
  ...
  this._pingRoot(function (err, data) {
    ...
    if (!self.transport.isConnected()) {
      self.transport.connect(data.endpoints.push, cb);
...
if (require.main === module) {
  ...
  require('../Utility.js').overrideConsole();
